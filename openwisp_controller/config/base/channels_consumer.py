...
class BaseConsumer(WebsocketConsumer):
    ...
    def _user_has_permissions(self, add=True, view=True, change=True, delete=True):
        permissions = []
        model_identifier = '{0}.{1}_{2}'.format(
            self.model._meta.app_label, self.model._meta.model_name, '{permission}'
        )
        if add:
            permissions.append(model_identifier.format('add'))
        if change:
            permissions.append(model_identifier.format('change'))
        if delete:
            permissions.append(model_identifier.format('delete'))
        if view:
            permissions.append(model_identifier.format('view'))
